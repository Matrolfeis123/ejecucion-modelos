...
def optimizar_año_pv_bess(año, SoC_inicial, parametros_planta, CoD, generacion_list, df_cmg):
    ...
    for t in t_indices:
        ...
        # Restricción de descarga con prorrateo entre los años 10 y 15
        capacidad_descarga_reducida = bess_discharge_power

        # if año > CoD + 15:
        #     capacidad_descarga_reducida = bess_discharge_power / 2


        model += (
            D_t[t] <= capacidad_descarga_reducida,
            f"Max_Descarga_{t}"
        )
...
def sensibilidad_pv_bess(parametros_planta_base, SoC_inicial, CoD, vida_util_proyecto, df_cmg, peak_power_values, capacidad_values, path_carpeta_output):
    ...
    for peak_power in peak_power_values:
        ...
        generacion_df = pd.read_csv(f'Terrazas_V2_INFORME/CASO_ESPERADO/generacion_terrazas_{peak_power}MWp_factor29.csv', sep=';')
        ...
        for capacidad in capacidad_values:
            ...
            filename = f"{path_carpeta_output}/output_pv_{peak_power}MW_bess_{capacidad}MWh_{parametros_planta["bess_charge_hours"]}hrs_sin_prorrateado_precioesp_factor29.xlsx"
            try:
...
if __name__ == "__main__":
    ...
    path_cmg = 'Terrazas_V2_INFORME/CASO_ESPERADO/CMg_terrazas_casoesperado.csv'
    ...
    capacidad_values = [920]


    # Carpeta de salida 
    path_carpeta_output = 'Terrazas_V2_INFORME/CASO_ESPERADO/'
