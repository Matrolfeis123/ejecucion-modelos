...
def formatear_df_cmg(path_csv: str, año: int):
    ...
    return df_cmg_año, costos_marginales



def optimizar_año(año, SoC_inicial, parametros_planta, CoD, generacion_list):
    ...
    # Leer el archivo de costos marginales y formatear el DataFrame
    path_csv = 'CMg.csv'
    df_cmg_año, costos_marginales = formatear_df_cmg(path_csv, año)
    ...
    # Vamos a aplicar degradacion acumulada a todos los valores mayores que 0, mientras que a los otros se mantienen igual
    ...
    PV_grid_t = pulp.LpVariable.dicts('PV_grid_t', t_indices, lowBound=-0.015, upBound=nominal_power, cat=pulp.LpContinuous)
    ...
    if pulp.LpStatus[model.status] == 'Optimal':
        ...
        #     'Generacion_PV_sin_Degr': [generacion_año[t] for t in t_indices],
...
generacion_df = pd.read_csv('generacion.csv', sep=';')
...
for i in range(len(generacion_list)):
    ...
    generacion_list[i] = max(-0.015, float(generacion_list[i]))


# Estado inicial de carga (SoC)
SoC_inicial = 0  # Puede ser otro valor si se requiere

# Ejecutar la optimización para múltiples años
# for año in range(CoD, CoD + vida_util_proyecto):
#     print(año)
#     SoC_final, resultados = optimizar_año(año, SoC_inicial, parametros_planta, tasa_degradacion_pv, tasa_degradacion_bess, CoD, generacion_list)
#     if resultados is not None:
#         # Guardar resultados en un archivo
#         resultados.to_excel(f'resultados_optimizacion_{año}.xlsx', index=False)
...
# Vamos a crear un ciclo que ejecute la optimizacion para cada año, y que almacene los resultados en un unico excel, concatenando los resultados de cada año

resultados = pd.DataFrame()
for año in range(CoD, CoD + vida_util_proyecto):
    print(año)
    SoC_final, resultados_año = optimizar_año(año, SoC_inicial, parametros_planta, CoD, generacion_list)
...
# Guardar resultados en un archivo
try:
    resultados.to_excel('resultados_optimizacion_25_AÑO_con_Degradacion_y_FO_NVA.xlsx', index=False)

except:
    print('Error al guardar los resultados en el archivo, debes cerrar el archivo')
    breakpoint()
    resultados.to_excel('resultados_optimizacion_25_AÑO_con_Degradacion_y_FO_NVA_penal.xlsx', index=False)
